...
$(".simworkarea").show()
if(dataRecd[currQues].dropped == true)
	{
		$(".maximizeIcon").hide()
	}
	else
	...
	if(cloneArr.length==0){
		...
		$('.simbtn,.crossbtn, .inspectSimbtn, .ticklebtn').css('background','#5a4a42').css('cursor','pointer').css('opacity','1');
...
/*	console.log(prevCard);
	...
	// $('.uncoded').empty();
	checkUncodedCards();
	// $($('.simworkcontainer .uncoded').clone()).appendTo('.uncoded');
	// $('.uncoded').append(dataRecd[[0]].uncodedcard)
	// console.log(dataRecd[[0]].uncodedcard);
	$('.checklistNo').html((curVal+1)+' of '+tempClone.length)
	$('#scaleCrdNo').html('Record '+(curVal+1)+' of '+tempClone.length);
...
function checkUncodedCards(){
$('.simworkcontainer .uncoded').css({display:'none'});
if(typeof dataRecd[0].uncodedcard!= 'undefined')
	{
	
	$.each(dataRecd[0].uncodedcard,function(i,v){
		if(v.dropped == false){
			$('.simworkcontainer #uc'+(i+1)).css({"display":"block"});
			retrieveUncodedData($('.simworkcontainer #uc'+(i+1)),v)
		}
else
{
console.log(dataRecd[0].uncodedcard)
...
var storingUncodedNum = function(){
	$('.simworkcontainer .uncoded .uncodedunit').off('keyup change').on('keyup change',function(e,ui){
		var tem = $(this).parent().closest('.uncoded').attr('id').slice(2);
		console.log(tem)
		if(typeof dataRecd[0].uncodedcard[tem-1] != 'undefined'){
		if(typeof dataRecd[0].uncodedcard[tem-1].uncodedValue== 'undefined')
		{
		dataRecd[0].uncodedcard[tem-1].uncodedValue = new Array();
		}
		dataRecd[0].uncodedcard[tem-1].uncodedValue = $(this).val();
		}
		// console.log($(this).parent().closest('.uncoded').attr('id'))
	})
}
var retrieveUncodedData = function(appenDiv,ucValue){
appenDiv.find('.ucData').remove();
appenDiv.find('.uncodedunit').val(ucValue.uncodedValue);
// console.log(ucValue.uncodedValue)
$.each(ucValue.uncards[0],function(i,v){
//appenDiv.append('<div class="ucData"></div>');
	$('<div/>',{id:'ucdata_'+i,class:'ucData'}).appendTo(appenDiv).text(v);
})
//appenDiv.find('.uncodedunit').val(ucValue.uncodedValue)
...
var hideSimArea = function(){
	...
	if(dataRecd[curVal].dropped){
		$('.simworkarea,.simworkarea .mCustomScrollBox').css({visibility:'hidden',display:'none'});
	}else{
		$('.simworkarea,.simworkarea .mCustomScrollBox').css({visibility:'visible',display:'block'});
	}	
}	
...
var accesslogfun = function(){

	     accessArr = [];
          $('.accesslog .access_unit').each(function(i,v){
	          var rest= $(this).val();
			  accessArr.push(rest);  
		  });
...
var keyPressOnAccesslog = function(){
	
$('.access_unit').off('keyup change').on('keyup change',function(e,ui){
...
$('#ticklename,#tickletextarea,#tickleRecordName').off('keyup change').on('keyup change',function(e,ui){
	...
	$('.simworkcontainer .tickler').change(function(){	
		var insertTickleUnit = $('.tickle-form .tickler').index($(this));
...
var splitName = function(getName,addClass,addLabel) {
	...
	splVal = getName;	
	$(addClass).empty();
	$('.checklistContent .mCSB_container').empty();
	if(dataRecd[curVal].job != 1)
	{   
	    playhint = 'hint_'+curVal;
        startToHintPlay(playhint,0)
	 for(var i=0; i<dataRecd[curVal].checklister.length; i++){
 		$('<div/>',{class:'rowDiv',id:'rowDiv_'+i}).appendTo('.checklistContent .mCSB_container');
		$('<div/>',{class:'checkboxDiv',id:'checkboxDiv_'+i}).appendTo('#rowDiv_'+i);
		$('<input>').attr({type:'checkbox',value:'checkbox_'+curVal+'_'+i}).appendTo('#checkboxDiv_'+i);
		$('<div/>',{class:'contentDiv',id:'contentDiv_'+i}).appendTo('#rowDiv_'+i).html(dataRecd[curVal].checklister[i]);
	 }
	 
	 if(typeof curchecklistVal[curVal] == 'undefined'){curchecklistVal[curVal] = [];}
	 $.each(curchecklistVal[curVal],function(index, value){
		 $("input[type=checkbox][value="+value+"]").prop("checked",true);
	 })
	 }
	 // $(".checkboxDiv input:checkbox").attr('checked','true');
	 checklistAct();
	   $(".scrollBarHint").mCustomScrollbar({
	   			theme:"3d-dark",
	   			axis:"y",
	   			advanced: {
		...
		if(	splVal.alignClass[index].indexOf('substeping')> -1){
			$('<div/>',{id:'latteradd'+index, class:splVal.alignClass[index]}).appendTo(addClass+' #latteradd'+(index-1));
			       if($('.simworkarea').hasClass('scaleCard') && addClass.indexOf('nameing') > -1){
						...
						var spLeft = $(addClass+' #latteradd'+(index-1)).position().left/0.28;
						var spTop = $(addClass+' #latteradd'+(index-1)).position().top/0.28;
					}
					else{
			...
			$(addClass+' #latteradd'+index).css({position:'absolute',right:'9px',top:0+'px'});			 
		...
		$.each(value,function(ind,val){
			...
			else if(splVal.alignClass[index].indexOf('SubInputTop')>-1){
			$('<div/>',{id:'namesub_'+index+'_'+ind,class:'names nameing_'+index}).appendTo(addClass+' #latteradd'+index).html('<div ><form><div> <div class="fieldDiv"> <div class="TopInputRight"><input type="text" style="width:138px;" name="userid"></div></div></div></form></div>');
				$(addClass+' #latteradd'+index).find('.TopInputRight input').attr('value',splVal.subTopInput);
			}
			else
			{
	...
	$('.simworkarea .TopInputRight input').off('keyup change').on('keyup change',function(e,ui){	
	
		dataRecd[curVal].subTopInput = $(this).val();
