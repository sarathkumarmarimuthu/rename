...
//$('.cover').css({position:'fixed',width:window.outerWidth+'px',height:window.outerHeight+'px',background:'blue'});
	...
	hoverEffect();
	$('.accessbtn').off('click').on('click', accessBtnEventCalling);
	$('#cnfmsubmit').off('click').on('click', reviewAns);
	$('.sitemapbtn, .fbtn, .btn, .subbtn, .backBtn, #startbtn, .clkLink, .generalinfobtn, .simfilebtn').bind('click', buttonEventCalling);
    $('.ticklebtn').off('click').on('click', ticklerFileBtnEventCalling);
	$( ".checklstbtn" ).off('click').on('click',checklistFun);
	...
	$('<div/>',{class:'draggerForParent'}).appendTo('.accesslogContainer');
	$('<div/>',{class:'chkdraggerForParent'}).appendTo('.checklistContainer');
  ...
  if(ltiexit)
	...
	if(cloneArr.length>0){
		$('[name = submitconfirmation]').removeClass('subbtnActive').addClass('simfilebtn').css({opacity:'0.5'});
		$('.dummySubbtn').show();
		$('.hideAudio').hide();
	}else{
...
/*	var Guidemodal = document.getElementById('createGuide');
    ...
    window.onclick = function(event) {
	 ...
	 /* $('.accessMin').unbind('click').bind('click',function(e){
		e.stopPropagation();
		var imgPath = $('.accessMin').css('backgroundImage');
		imgPath = imgPath.substr(imgPath.lastIndexOf('/')+1,imgPath.length).split('.')[0]
		console.log(imgPath+" minimize");
		if(imgPath == "maximize")
		{
			
			$('.accesslogContainer').stop().animate({height:'30px'},function(){	
			});	
			$('.accessMin').attr('data',1);
			$('.accessMin').css({background: 'url(./assets/images/minimize.png) no-repeat'});						
		}
		else
		{
			var topval = parseInt($('.accesslogContainer').css('top'))
			var leftval = parseInt($('.accesslogContainer').css('left'))
			if(topval > 185 ) {topval = 185}
			if(leftval > 695 ) {leftval = 695}
			{
			$('.accesslogContainer').stop().animate({height:'370px',opacity:'1'},function(){
		
			})
			}
			$('.accessMin').attr('data',0);
			$('.accessMin').css({background: 'url(./assets/images/maximize.png) no-repeat'});	
			
		}
		}); */
		
	    $('.accessMin').off('click').on('click',function(e){
		
		if(parseInt($('.accesslog').css('height')) >= 250)
		  {
			$('.accesslog').animate({height:'0px', width:'430px'},function(){});
			$('.accesslogContainer').css({height:'30px', width:'450px'});
			$('.accessMin').attr('data',1);
			$('.accessMin').css({background: 'url(./assets/images/minimize.png) no-repeat'});	
			$('.accesslogContainer').css({'box-shadow':'0 5px 6px 0 #999'});
		  }
		  else
		  {
			var topval = parseInt($('.accesslog').css('top'))
			var leftval = parseInt($('.accesslog').css('left'))
			if(topval > 185 ) {topval = 185}
			if(leftval > 695 ) {leftval = 695}
			{
				$('.accesslog').animate({height:'370px', width:'430px', top:topval+'px', left:leftval+"px"},function(){})
				$('.accesslogContainer').css({height:'400px', width:'450px'});
				$('.accesslogContainer').css({'box-shadow':'none'});
			}
			$('.accessMin').attr('data',0);
			$('.accessMin').css({background: 'url(./assets/images/maximize.png) no-repeat'});	
	   ...
	   $('.accessClose').off('click').on('click',function(e){
		   $('.accesssubmit').css({'background':'#5A4A42','color':'#fff'});
		   $("input.access_unit").val("");
		   $.each(accessArr,function(index, value){
			   $("input.access_unit").eq(index).val(value);
		   })
           $('.accesslogContainer').css('visibility','hidden');
	   });


      $('.checklistMin').off('click').on('click',function(e){
		
		if(parseInt($('.checklistContainer').css('height')) >= 100)
		  {
			$('.checklistContainer').animate({height:'30px', width:'198px'},function(){});
			
			$('.checklistMin').attr('data',1);
			$('.checklistMin').css({background: 'url(./assets/images/chklist_minimize.png) no-repeat'});	
		  }
		  else
		  { 
			var topval = parseInt($('.checklistContainer').css('top'))
			var leftval = parseInt($('.checklistContainer').css('left'))
			if(topval > 290 ) {topval = 290}
			if(leftval > 795 ) {leftval = 795}
			{
				$('.checklistContainer').animate({height:'240px', width:'198px'},function(){})
			  
			}
			$('.checklistMin').attr('data',0);
			$('.checklistMin').css({background: 'url(./assets/images/chklist_maximize.png) no-repeat'});	
...
$('.dragtooldiv,.dragarea').off('touchmove').on('touchmove',function(e){					
	...
	totcodescore = (dataRecd.length+uncodedCards.length)
}
/*
...
var createTickleFolders = function(folderArr){
	...
	tickletypeandName.forEach(function(item,index)	
	{
		...
		$('.dragareatickle').prepend('<div id=dragtooldivguide_'+index+' class="dragtooldiv guide unsortable"><span></span><div class="addguide">'+(tickletypeandName[index][1])+'</div><span class="drogtoolname">'+(tickletypeandName[index][1])+'</span><span class="folderindex">'+0+'</span><span class="drogtoolnum">'+index+'</span><div class="dummyguide">'+(tickletypeandName[index][2])+'</div><span class="rigCurveg"></span></div>');
			...
			if(tickletypeandName[index][1].length>7)
			{
...
var hoverTopContainer = function(){
	...
	$( ".dragareatickle .addspecialguide,.dragareatickle .addguide,.dragareatickle .dummyspguide,.dragareatickle .dummyguide").droppable({tolerance:'pointer',hoverClass:'hvr',accept:'.crossreference ,.simworkarea, .ticklefile,.uncoded',out:function(){
		//		console.log('out')
	}
		, over:function (){
...
var hoverDownContainer = function(){
	...
	$( ".dragarea .addfolder,.dragarea .dummyFolder" ).droppable({tolerance:'pointer',hoverClass:'hvr',accept:'.crossreference ,.simworkarea, .ticklefile,.uncoded',out:function(){
...
var navigateFolderMain = function(){
		...
		$('.rightarraw').droppable({
			...
			accept:'.dragtooldiv,.crossreference ,.simworkarea,.ticklefile,.uncoded'
	});
...
var dependsOn = function(cArr,g){
	...
	onCheckbox();
	if(currQues==0){console.log(currQues)
		 $('.prev,.prevmin,.next,.nextmin,.startprev,.endnext').off('click').css({opacity:'0.5',cursor:'default'}); 
		 $('.next,.nextmin').off('click').on('click', getNext).css({opacity:'1.0',cursor:'pointer'});
		//var cv = checkboxValidation(); 
		
		 if(!ltiexit)
		 $('[name = submitconfirmation]').removeClass('subbtnActive').addClass('simfilebtn');
		 if(jobTwoLen==1){console.log(currQues)
			 $('.next,.nextmin,.startprev').off('click').css({opacity:'0.5',cursor:'default'});
			 $('.prev,.prevmin,.endnext').off('click').css({opacity:'0.5',cursor:'default'});
		 }
	 }
	else if(currQues==jobTwoLen-1){console.log(currQues)
	...
	}else{console.log(currQues)
		$('.next,.nextmin').off('click').on('click', getNext).css({opacity:'1.0',cursor:'pointer'});
		$('.prev,.prevmin').off('click').on('click', getPrev).css({opacity:'1.0',cursor:'pointer'});
		$('.startprev').off('click').on('click', getStartPrev).css({opacity:'1.0',cursor:'pointer'});

	}
	testingCheckBox()
	$('.checklister').find('input:checkbox').off('change').change(function() {
	if($(this).is(':checked'))
           {
             var checkedVal = $(this).val();
			     curchecklistVal[curVal].push(checkedVal);
				 
           }
		   else
		   {
			   var chkIndex = curchecklistVal[curVal].indexOf($(this).val());
			   if (chkIndex > -1) {
				curchecklistVal[curVal].splice(chkIndex, 1);
}
		   }
	testingCheckBox()
	})
}
var testingCheckBox = function(){
return;
	if(curchecklistVal[curVal].length == dataRecd[currQues].checklister.length && typeof dataRecd[currQues].dropped !== 'undefined')
	{
	
		$('.next,.nextmin').off('click').on('click', getNext).css({opacity:'1.0',cursor:'pointer'});
		if(currQues==(dataRecd.length-1))
		{
			$('.next,.nextmin').off('click').css({opacity:'0.5',cursor:'default'}); 
		}
	}
	else if(currQues==(dataRecd.length-1))
	{
		$('.next,.nextmin').off('click').css({opacity:'0.5',cursor:'default'}); 
	}
	else
...
var onCheckbox = function(){
	$('.checklister').eq(currQues).find('input:checkbox').change(function(i,v){
		var chv = checkboxValidation();
		if(chv)
		dependsOn();
	});
}
var checkboxValidation = function(){
	var cc = 0;
	$('.checklister').eq(currQues).find('input:checkbox').each(function(i,v){
		if($(this).prop('checked')){
			cc++;
		}
	});
	// console.log($('.checklistContent input:checkbox').length+' '+cc)
	// if(cc == $('.checklistContent input:checkbox').length){
		 // $('.next,.nextmin').off('click').on('click', getNext).css({opacity:'1.0',cursor:'pointer'});
	// }else{
		 // $('.next,.nextmin').off('click').css({opacity:'0.5',cursor:'default'});
	// }
		 
	return cc == $('.checklistContent input:checkbox').length?true:false;
}
...
curNav = 'none';
	...
	if(editcurrQues!=0){
	$('.endnext').off('click').on('click', getendnext).css({opacity:'1.0',cursor:'pointer'});	
	}
	else{
	$('.endnext').off('click').css({opacity:'0.5',cursor:'default'}); 
	}
}
var getendnext = function() {
currQues=editcurrQues;
chngQues(currQues); 
	
	
	 $('.endnext').off('click').css({opacity:'0.5',cursor:'default'}); 
	 console.log(editcurrQues+'     '+currQues)	
}
...
var getNext = function() {
	...
	hideshowXRex();
	$.each(curchecklistVal[curVal],function(index, value){
		 $("input[type=checkbox][value="+value+"]").prop("checked",true);
	 })
	testingCheckBox();
	var str = $('.checklistNo').html()
	// console.log((+str.substr(0,str.indexOf(' ')))+1);
...
var getPrev = function() { 
	...
	var str = $('.checklistNo').html()
	 //console.log((+str.substr(0,str.indexOf(' ')))+1);
	// $('.checklistNo').html(((+str.substr(0,str.indexOf(' ')))-1)+' of '+tempClone.length)
	// $('#scaleCrdNo').html('Record '+((+str.substr(0,str.indexOf(' ')))-1)+' of '+tempClone.length);
	$('.endnext').off('click').on('click', getendnext).css({opacity:'1.0',cursor:'pointer'});		
}
...
var showFolder = function(){
	...
	$('.containersubheadone').css({marginLeft: '-63px',marginTop: '-23px',fontSize: '19pt'});	
	$('.crossreferenceContainter, .tickleContainer').css({height:'320px'});
	$('#inspectid').hide();
	$('#tooltip').hide();
	$('#noOFCrds, #noofTicklecrds').hide();
...
//console.log(currQues+' '+crosCount+' '+cloneCnt)
	...
	$('#scaleCrdNo').html('Record '+(tempClone.indexOf(cloneArr[cloneCnt])+1)+' of '+tempClone.length);
	$('.checklistNo').html((tempClone.indexOf(cloneArr[cloneCnt])+1)+' of '+tempClone.length);
...
var accessBtnEventCalling = function(){
	
    	if($('.accesslogContainer').css('display') == 'none'){
		     $('.accesslogContainer').css('display','block');
			 $('.accesslog .access_unit').val('');
			 keyPressOnAccesslog();
			 $('.accesssubmit').off('click').on('click', accesslogfun);
		}
		if($('.accesslogContainer').css('visibility','hidden')){
			$('.accesslogContainer').css('visibility','visible')
		}
}
...
var curchecklistVal=[];
var checklistFun = function(){
  
  if($('.checklistContainer').css('display') == 'none'){
...
var onchecking = function(){
	 ...
	 $(".checkboxDiv input:checkbox").off('change').on('change',function() {
...
//			alert(currentidarr);
	...
	$('#closePopup').hide();
	   ...
	   if(currentId!='empty'){   //playAudio(currentId,0);
	     ...
	     $('.brandIcon').css('cursor','default');
		 $('.TopInputRight input').prop( "disabled", true );
	     $('.fbtn p').eq(0).css({opacity:'0.5'}).css('cursor','default');
	   ...
	   $('.checklistContainer').css('display','none')
		...
		// console.log(codescore, folderscore, inspectscore, folderGuideextraScore, accessScore)      
        // console.log(totcodescore, totfolderscore, totinspectscore, (totalfolderScore*20), totaccessScore)
		$.each(uncodedCards,function(ni,nv){
		codescore += uncodedCards[ni].uncardcoding
		
		})
		recordSimScore = (((Number(codescore) + Number(folderscore) + Number(folderGuideextraScore)+ Number(accessScore)) / (Number(totcodescore) + Number(totfolderscore) + Number(totaccessScore)  + Number(totalfolderScore*2))) * 100).toFixed(2);
		
		
		// recordSimScore = (((Number(codescore) + Number(folderscore)) / (Number(totcodescore) + Number(totfolderscore) + Number(totalfolderScore*2))) * 100).toFixed(2);
