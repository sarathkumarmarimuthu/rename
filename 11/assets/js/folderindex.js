...
var enableDrag = function() {	
	...
	$('.crossreference ,.simworkarea, .ticklefile').draggable({
								...
								$('.simworkcontainer .uncoded').draggable({
								helper:'clone',
								cancel:'.uncodedunit',
								handle:'.ucData',
								revert :function(event, ui) {
								           var draggedCardId = $(this).attr('id').slice(2);
											$(this).data("uiDraggable").originalPosition = {
													top : $('.simworkcontainer #uc'+draggedCardId).offset().top-35,
													left : $('.simworkcontainer #uc'+draggedCardId).offset().left-50,					
													transform:'scale(0.6)'
												};
											return !event;
								},
		...
		if(navigator.userAgent.indexOf('Mac OS X') != -1) {
         ...
         $('.tickleContainer').draggable({containment:'.cover',start:function(){
										
										},handle:'.draggerForParent,#noofTicklecrds'});	
										
	    // $('.checklistContainer').draggable({containment:'.cover',start:function(){
		//								 },handle:'.chkdraggerForParent'});											
		 $('.accesslogContainer').draggable({containment:'.cover',
		                                    start:function(){ /*$('#dummyAccessDiv').show();*/},handle:'.draggerForParent,.accessTitle',stop:function(){ /*$('#dummyAccessDiv').hide();*/},	cancel:''});										
...
var storeUncodedCards = function() {
	...
	var folLoc, folOrder;
		...
		if(typeof uncodedCards!= 'undefined') {
			
			
			$.each(uncodedCards,function(inx,v){
			
				var crossAnsOdr = v;
				//v.order = '0,0';
				var insCrsInp = v.uncards;			
		
				folLoc = v.xfolderinfo[0][0];
				responseTo = getElement(folLoc);
				folOrder = v.xfolderinfo[0][1];	
...
var storeJobOne = function() {
	...
	if(value.folderinfo.length>0){ 
		...
		//alert(recordIndex[[folLoc]].length+'crf')				
				...
				if(typeof(value.crossUnit[inx])=="undefined"){
					value.crossUnit.push({arr:[],job:1,xref:'x',dropped:true,crosRefCount:0});					
				}
				for(i=0;i<insCrsInp.length;i++){
		...
		//alert(recordIndex[[folLoc]].length+'crf')				
				...
				if(typeof(value.tickleUnit[inx])=="undefined"){
					value.tickleUnit.push({arr:[],job:1,xref:'t',dropped:true});					
				}
...
var setCursor1 = function(e,ui) {}
var setCursor = function(e,ui) {	
$('.uncodedunit').removeAttr('autofocus').blur(); 
//console.log('ssss');
		starttag = true;
		chkOnDrag=true;
	  // shwCurrentElem();
		 ...
		 if($(this).hasClass('simworkarea'))
		 {
            $(this).draggable('instance').offset.click = {
                 left: Math.floor(ui.helper.width() / 2) + 60,
			     top: Math.floor(ui.helper.height() / 2) + 110
            }; 	
}
else if($(this).hasClass('uncoded'))
{
         $(this).draggable('instance').offset.click = {
            left: Math.floor(ui.helper.width() / 2),	
			top: Math.floor(ui.helper.height() / 2) + 47	
         };
}		 
...
var changCursor1 = function(e,ui) {
	$('.uncodedunit').removeAttr('autofocus'); 
var topstartpoint = 400;
	...
	// console.log(scalecard,xposition,e.pageX,e.pageY,holdUiPos)
}
...
var changCursor = function(e,ui) {
	...
	{		
		...
		if(xaxis > 370){scalecard = 0.153;}	
		if(scalecard>0.28){
			scalecard = 0.28;
		}	
	}else{
		var scalecard = Math.abs(parseInt(xaxis)-1500)/1500;		
		if(xaxis > 370){scalecard = 0.28;}
	}	
	if(xaxis>980){
...
//		console.log('dragarea');
		...
		},accept:'.swapAreaDrop,.crossreference,.simworkarea, .ticklefile,.uncoded',drop:dropfolderindex,tolerance:'pointer'});
}
...
//	indexId=$(this).children().eq(2);	
	...
	}else if($(getClass).hasClass("uncoded"))
	{
	var tem = $(getClass).attr('id').slice(2);
	
	
	if(getInx == 4)
	{
	dataRecd[0].uncodedcard[tem-1].uncodedValue = "";
	}
	dataRecd[0].uncodedcard[tem-1].dropped = true;
	recordIndex[[getInx]].push(dataRecd[0].uncodedcard[tem-1]);
	// console.log(dataRecd[curVal].uncodedcard)
	dataRecd[0].uncodedcard.splice(tem-1, 1)
	checkUncodedCards()
	//alert(temp-1);
	// $('.simworkcontainer #uc'+(tem)).css({display:"none",background:'green'});
	//$('.simworkcontainer #uc'+(tem)).remove();
...
//	console.log(recordIndex)
	...
	testingCheckBox();
	// var tight = 0;
	// $.each(dataRecd,function(ii,vv){
	// if(typeof dataRecd[0].uncodedcard!= 'undefined')
	// {
	// tight++;
	// $.each(dataRecd[0].uncodedcard,function(i,v){
	// console.log(dataRecd[0].uncodedcard[i].dropped)
	// })
	// }
	// })
	// console.log(tight)
	if(cloneArr.length==0 && ($('.simworkcontainer #uc1').css('display') == 'none') && ($('.simworkcontainer #uc2').css('display') == 'none')){
		$('[name = submitconfirmation]').removeClass('simfilebtn').addClass('subbtnActive');
		$('.offSteps').hide();	
	}
}
		...
		if(typeof value !== 'undefined'){		
			$.each(value,function(inx,val){
				if(typeof val.name != 'undefined'){
					...
					val.folderLocation = [index,inx];					
					uncodedCards[val.pos].folderLocation = [index,inx];
					//console.log(val.folderLocation+' '+val.uncards[0][1]);
				}
			});
...
var checkFinalCard = function() {
	...
	if (uniqArr.length!=1 && dataRecd[curVal].dropped) {
		...
		if(clost.length==0 && tickleclost.length==0){
			...
			//$('[name = submitconfirmation]').removeClass('subbtnActive').addClass('simfilebtn');
			$('.simworkcontainer').show();
			$('.hideBtns').hide();
			$('.dummySubbtn').show();
...
//			console.log(JSON.stringify(recdIndVal));
	...
	{
		$('.panelContainer .swapAreaDrop,.panelContainer .swapAreaDrop .cross_unit,.panelContainer .swapArea .tickler').css('cursor','ew-resize')
	...
	$('.panelContainer .job1 .swapArea').css({transform:'scale(0.98)',transformOrigin:'0 0'});
	$('.panelContainer').sortable({start:getPrevIndex,tolerance: 'pointer',sort:changCursor2,stop:restScl,update:swapOrder,refreshPositions: true,cancel:'.swapOrderNums,.dragCrd',handle:'.swapArea',axis:'x'});
...
var zoomCard = function(){
  ...
  var selectedval2;
		...
		if($('.parentCardPanel').is(":visible")){	
			$($(this).parent().clone()).appendTo('#cloneContainer');
			    $('.tickle_unit .tickler').each(function(index,value){	
			    		    selectedval = $('.tickle_unit .tickler').eq(index).val(); 
			    			$('#zoompopup .tickle_unit .tickler').eq(index).val(selectedval); 
		         });
			if(!$("#cloneContainer .swapArea").children().hasClass('form') && !$("#cloneContainer .swapArea").children().hasClass('tickle-form')&& !$("#cloneContainer .swapArea").children().hasClass('uncoded'))
			{
			...
			if(1){
				...
				setTimeout(function(){$('#zoompopup #cloneContainer').find('#close2').css({'top':'20px','right':'283px'});},5)
	...
	$("#cloneContainer .simworkcontainer").css({transform:'scale(1.8)',left:'132px',top:'190px'});
...
$('.dummypanelCover').show();			
	...
	if(getInx == 4)
	{
	$('.panelContainer input').css({visibility:'hidden'});
	}
	
	}
	$('.panelContainer  .uncodedunit').removeAttr('autofocus');   
	}
}
...
if(ctl == 1 || inDevice){  
	...
	if($(this).hasClass('codeEditIcon') ){	
		...
		editDatInd = $(this).closest('.swapAreaDrop').index();
		e.stopPropagation();
		if(typeof(recordIndex[[currAlphabet]][editDatInd].order) != 'undefined'){
		...
		else if( recordIndex[[currAlphabet]][editDatInd].xref == 'x'){				
			...
			finalChk = 'xref';	
		}else if( recordIndex[[currAlphabet]][editDatInd].xref == 't') {				
			removedElem = recordIndex[[currAlphabet]][editDatInd].arr[9];	
			...
			recordIndex[[currAlphabet]][editDatInd].folderLocation = [];			
			recordIndex[[currAlphabet]].splice(editDatInd,1);
			orginalPos = removedElem.split(',');alert(crosCount);
			
			dataRecd[orginalPos[0]].tickleUnit[crosCount].dropped = false;
			//placeItBack();	//to remove crossref with maincard from folder
			finalChk = 'tick';	
		}else{
		if(typeof dataRecd[0].uncodedcard!= 'undefined' && dataRecd[0].uncodedcard.length >= 2)
		{
			$('.alertcnt').show();
			$('.dummyalertCover').show();
			$('.alertclose').off('click').on('click',function(){
                $('.alertcnt, .dummyalertCover').hide();
            });

			return;
		}
			orginalPos = [currQues,currQues];
			if(typeof dataRecd[0].uncodedcard == 'undefined'){
				dataRecd[0].uncodedcard = [];
			}
			recordIndex[[currAlphabet]][editDatInd].dropped = false;
			dataRecd[0].uncodedcard.push(recordIndex[[currAlphabet]][editDatInd]);
			// console.log(recordIndex[[currAlphabet]][editDatInd])
			// console.log(editDatInd)
			
			recordIndex[[currAlphabet]][editDatInd].folderLocation = [];			
			recordIndex[[currAlphabet]].splice(editDatInd,1);
			//dataRecd[orginalPos].uncards[].dropped
		}
		checkRecdIndex();
		checkEmptyFolder(currAlphabet,'curr');		
	}	
...
if($(this).hasClass('codeEditIcon')){
   ...
   if($('#alphabet').text() == 'UNCODED CARDS')
	{
	$('.panelContainer input').val('');
	$('.panelContainer input').css({visibility:'hidden'});
	}
		...
		if(typeof(value) != 'undefined' && value !== null){
			...
			{			
					if(typeof(v.order) != 'undefined'){
						...
						if(v.xref == 'x' && typeof(v.arr[9]) != 'undefined'){	
...
var swapInside = function(i,objVal,appendClass) {	
	...
	$('.editclk').attr('title', 'Return card to deck');
...
var addCrossRef = function(i,objVal,appendClass,isAns) {
	...
	$('<div/>',{class:'dragCrd'}).appendTo(appendClass+' #swapAreaDrop_'+i);	
	$('.crossClk').attr('title', 'Return card to deck');
...
var addTickleCard = function(i,objVal,appendClass,isAns) {
	...
	$('<div/>',{class:'offFocus'}).appendTo(appendClass+' #swapArea_'+i);	
	$('<div/>',{class:'dragCrd'}).appendTo(appendClass+' #swapAreaDrop_'+i);	
	$('.tickleClk').attr('title', 'Return card to deck');
		...
		if(isAns != 'notAns' && typeof tickleAnsOdr != 'undefined'){
			...
			if(isAns != 'notAns'){
				...
				if(index != 2 && index != 3  && index != 4 ){
					...
					$(appendClass+' #swapArea_'+i+' .tickler').eq(index).text($.trim(objVal.arr[index]));
				}			
...
var addUncodedCards = function(i,objVal,appendClass,isAns){
	$('<div/>',{id:'swapAreaDrop_'+i,class:'swapAreaDrop swapAreaDropUncoded'}).appendTo(appendClass).html();
	$('<div/>',{id:'swapArea_'+i,class:'swapArea swapAreaUncoded'}).appendTo(appendClass+' #swapAreaDrop_'+i);
	/**/
	$('<div/>',{id:'uc'+i,class:'uncoded'}).appendTo(appendClass+' #swapArea_'+i);
	$('<div/>',{id:'uform'+i,class:'uncoded-form'}).appendTo(appendClass+' #swapArea_'+i+' #uc'+i );
	$('<div/>',{id:'u'+i,class:'uuu'}).appendTo(appendClass+' #swapArea_'+i+' #uform'+i);
	$('<input/>',{id:'uncodedname'+i,class:'uncodedunit'}).appendTo(appendClass+' #swapArea_'+i+' #u'+i);
	for(var j=0;j<4;j++){
		$('<div/>',{id:'ucdata_'+j,class:'ucData',type:'text'}).appendTo(appendClass+' #swapArea_'+i+' #uc'+i);
	}
	
	
	//$($('.simworkcontainer #uc1').clone()).appendTo(appendClass+' #swapArea_'+i);

	retrieveUncodedData($(appendClass+' #swapArea_'+i+' .uncoded'),objVal);
	if(isAns == 'refAns'){
	//console.log(JSON.stringify(objVal))
		$(appendClass+' #swapArea_'+i+' .uncoded').find('.uncodedunit').val(objVal.uncodedNum)
	}else{
		$(appendClass+' #swapArea_'+i+' .uncoded').find('.uncodedunit').val(objVal.uncodedValue)
	}
	
    $('.tickle-form select').addClass('tickler');
	$('<div/>',{class:'editclk uncodedClk'}).appendTo(appendClass+' #swapArea_'+i);
	$('<div/>',{class:'offFocus'}).appendTo(appendClass+' #swapArea_'+i);	
	$('<div/>',{class:'dragCrd'}).appendTo(appendClass+' #swapAreaDrop_'+i);	
	
	$(appendClass+' .swapArea .tickle-form').find('.tickle-form > div').attr('readonly','readonly');		
	//$(appendClass+' #swapArea_'+i+' .tickle_unit').html('');
	$('<div/>',{class:'cardNum'}).appendTo(appendClass+' #swapArea_'+i);
	editDataClk();
}
...
var pushUser = function() {
	...
	$('.userPanel,	.correctPanel, .codeResult,.orderResult').empty();	
		...
		if(typeof uncodedCards != 'undefined'){
						$.each(uncodedCards,function(ii,vv){						
							addUncodedCards(idx,vv,'.userPanel','notAns');
							addUncodedCards(idx,vv,'.correctPanel','refAns');
							createUncodedAns(idx,vv,'.codeResult','tickleCrd',vv.folderLocation[0],vv.folderLocation[1]);
		...
		for(var i=0;i<dataRecd.length; i++)
		{
          ...
          $.each(accesslogAnsarr, function(i,val) {
			for ( var i = 0; i < accessArr.length; i++ ) { 
			     if ( val === accessArr[i].trim().toString().replace(/  +/g, ' ')) {
             	    accessScore++;
                 } 
			 }
         });
		 
	$('<div/>',{id:'swapAreaDrop_'+(idx+1),class:'swapAreaDrop'}).appendTo('.userPanel').css({});	  
	$('<div/>',{id:'swapAreaDrop_'+(idx+1),class:'swapAreaDrop'}).appendTo('.correctPanel').css({marginTop:'0px'});
    $('<div/>',{id:'swapAreaDrop_'+(idx+1),class:'swapAreaDrop'}).appendTo('.codeResult');
	//.css({marginTop:'763px',marginLeft:'10px',background:'red',width:'900px',height:'70px'});
	$('<div/>',{id:'swapArea_'+i,class:'swapAreaccess'}).appendTo('.userPanel #swapAreaDrop_'+(idx+1));
	$('<div/>',{id:'swapArea_'+i,class:'swapAreaccess'}).appendTo('.correctPanel #swapAreaDrop_'+(idx+1));
	$('<div/>',{class:'accessCodeReult'}).appendTo('.codeResult #swapAreaDrop_'+(idx+1));
	$($('.accesslog .accessform').clone()).appendTo('.userPanel .swapAreaccess');
	$($('.accesslog .accessform').clone()).appendTo('.correctPanel .swapAreaccess');
	$('.userPanel .access_unit').val('');  
	$('.correctPanel .access_unit').val('');

		 	$.each(accesslogAnsarr, function(index,value){
                   $('.correctPanel .access_unit').eq(index).attr('value',value);
	               $('.correctPanel .access_unit').eq(index).val(value);
		   	}); 
			
			$.each(accessArr, function(index,value){
                   $('.userPanel .access_unit').eq(index).attr('value',value);
	               $('.userPanel .access_unit').eq(index).val(value);
		   	});
	 	
	if(accessScore == totaccessScore){
	 $('<span class="right" style="color:#999"><font style="color:green"><font style="color:#000" >Accession Log Result:</font> You correctly completed your Accession Log.</font><font color="#000"> Score: 6/6</font></span>').appendTo('.codeResult .accessCodeReult');
	}else {
	$('<span class="wrong" style="color:#999"><font style="color:red"><font style="color:#000" >Accession Log Result:</font> You incorrectly marked in your Accession Log.</font><font color="#000"> Score: '+accessScore+'/6</font></span>').appendTo('.codeResult .accessCodeReult');
	}
	
	
	$('.correctPanel #swapAreaDrop_'+(idx)).css({height:'400px'})
...
//   setHeight:'200px',
  ...
  callbacks:{whileScrolling:function(){		 
		  ...
		  if(this.mcs.topPct>=95){
			  $('#ctr0 .arrng1').html('Your Completed Accession Log'); 
			  $('#ctr0 .arrng2').html('Correct Completed Accession Log');
			  $('#ctr0 .overallScr').hide();
		  }else{
			 $('#ctr0 .arrng1').html('Your Choice'); 
			 $('#ctr0 .arrng2').html('Correct Answer');
			 $('#ctr0 .overallScr').show();
		  }
...
var createCode = function(inx, val, appclass) {
	...
	$('<div/>',{id:'cardIn_'+inx,class:'cardsChild'}).appendTo(appclass+' #card_'+inx);
		
	if(val.job == 1){
		$(appclass + ' #cardIn_'+inx).html('<span style="color:#999"; class="wrong"><font style="color:#000;font-weight:bold;">Coding Result: </font>N/A</span>');
	}else{	
			var gtxt = ' ';
			
		$('.correctPanel'+ ' #swapArea_'+inx+' .TopInputRight input').attr('value',val.numericCode);	
			
		
		if(val.subTopInput == val.numericCode){
			val.codeScore[0] = 1;
			$(appclass+ ' #cardIn_'+inx).html('<span class="right"><font style="color:#000;font-weight:bold;">Coding Result: </font> You correctly applied numeric coding. <font color="#000"> Score:&nbsp;'+val.codeScore[0]+'/'+val.codeScore[1]+'</font></span>');
			console.log()
		}
		else{		
			
			$(appclass+ ' #cardIn_'+inx).html('<span class="wrong"><font style="color:#000;font-weight:bold;">Coding Result: </font> Your numeric coding contains errors. <font color="#000"> Score:&nbsp;'+val.codeScore[0]+'/'+val.codeScore[1]+'</font></span>');
		}
	}
	...
	//$('<div/>',{id:'cardIn_'+inx+1,class:'cardsChild'}).appendTo('#card_'+inx);
}
...
var createUncodedAns = function(inx, val, appclass, tickleCrd, afol,ainx,getOrg) {
	$('<div/>',{id:'card_'+inx,class:'swapAreaDrop'}).appendTo(appclass);
	$('<div/>',{id:'cardIn_'+inx,class:'cardsChild'}).appendTo(appclass+' #card_'+inx);
// $('#cardIn_'+inx).html('<span style="color:#999"; class="wrong"><font style="color:#000;font-weight:bold;">Inspection Result: </font>N/A</span><span style="color:#999"; class="wrong"><font style="color:#000;font-weight:bold;">Coding Result: </font>N/A</span>');	
	if(typeof val.uncodedValue != 'undefined')
	{
		if(val.uncodedValue == val.uncodedNum)
		{
			$(appclass+' #cardIn_'+inx).html('<span class="right"><font style="color:#000;font-weight:bold;">Coding Result:</font> You correctly applied numeric coding.<font color="#000"> Score:&nbsp;1/1</font></span>');
			val.uncardcoding=1;				
		}
		else
		{
			$(appclass+' #cardIn_'+inx).html('<span class="wrong"><font style="color:#000;font-weight:bold;">Coding Result: </font> Your numeric coding contains errors.<font color="#000"> Score:&nbsp;0/1</font></span>');
			val.uncardcoding=0;	
		}
	}
	else
	{
	$(appclass+' #cardIn_'+inx).html('<span class="wrong"><font style="color:#000;font-weight:bold;">Coding Result:</font> You did not filled the numeric coding.<font color="#000"> Score:&nbsp;0/1</font></span>');
	val.uncardcoding=0;	
	}
}
...
var createOrder = function(inx, val, appclass,index,order){
	...
	}else if(typeof val !== 'undefined' && val.xref == 'x'){		
		var getOrg;
		var getCrossAns = val.arr[9].split(',');
		var crossAnsOdr = getCrossAns[1];
	...
	}else if(typeof val !== 'undefined' && val.xref == 't'){
		var getOrg;
		var getCrossAns = val.arr[9].split(',');
	...
	{	
		getFolOdr = val.uncards[1][0];
		getInsOdr = val.uncards[1][1];
		var getActOdr = val.uncards[1][2];			
		showOrders(index, inx, getFolOdr, getInsOdr, getActOdr, order, val);
	}
...
var showOrders = function(index, inx, getFolOdr, getInsOdr, getActOdr, order, val,getOrg) {
    ...
    crscoreassign(val, 0);
	var drawersep='';
	var checker = (twoReturns[1] == 'A')?'ALPHABETIC CARD FILE':'CORRESPONDENCE FILE'
//	console.log(fol+' '+getFolOdr+' '+index)
	var drawersep = index >= parseInt($('.dragareatickle').children().length) ? 'correspondence file drawer': 'ALPHABETIC CARD FILE';

	if(typeof getFolOdr != 'undefined'){
	var ansDrawersep = getFolOdr >= parseInt($('.dragareatickle').children().length) ? 'correspondence file drawer': 'ALPHABETIC CARD FILE';
    }
	...
	if(val.job != 1 && typeof val.arr != 'undefined' && typeof val.first == 'undefined'){
			...
			console.log(val.name+' '+getFolOdr+' '+getInsOdr)
			    var xrefTickle = val.xref == 'x' ?'extra cross-reference sheet':'extra tickler card';
...
//getting the card placed in ALPHABETIC CARD FILE drawer or CORRESPONDENCE FILE drawer 
	var responseTo;
	var drawerLabel;
//	console.log('placed '+rowIndex);
//alert(rowIndex+' '+parseInt($('.dragareatickle').children().length))
