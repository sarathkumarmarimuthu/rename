...
var chngQues = function(currQues) {	
	...
	$('[name = '+dataRecd[curVal].inspect+']').css('background','#e75028');
	
	
	for(var i = 0; i<dataRecd[curVal].subinputval.length; i++)
	{
		
		$('.subinput input:eq('+i+')').val(dataRecd[curVal].subinputval[i]);
	}
...
var crossReferAct = function() {
	...
	{
			$('.crossreferenceContainter').animate({height:'320px', width:'320px'},function(){$('#noOFCrds').html('CROSS-REFERENCE SHEET: '+ (parseInt(crosCount)+1) +' / '+dataRecd[curVal].crossUnit.length);})
	...
	$.each(splVal.name,function(index,value){
		...
		if(	splVal.alignClass[index].indexOf('substeping')> -1){
			...
			if($('.simworkarea').hasClass('scaleCard') && addClass.indexOf('nameing') > -1){
						...
						var spLeft = $(addClass+' #latteradd'+(index-1)).position().left/0.53;
						var spTop = $(addClass+' #latteradd'+(index-1)).position().top/0.53;
					}
					else{
						...
						console.log(index+' '+spTop+' '+spLeft)
					}

			$(addClass+' #latteradd'+index).css({position:'absolute',right:'27px',top:spTop+'px'});
		}
