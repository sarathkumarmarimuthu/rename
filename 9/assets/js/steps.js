			...
			//$.each(val,function(i,v){
				...
				if(inx!=0){			
					if($('.simworkarea').hasClass('scaleCard') && applyTo == '.nameing'){
						var getLeft = posOf.position().left/0.53;
						var getTop = posOf.position().top/0.53;
					}else{
					...
					if($(applyTo+' #latteradd'+splt[0]).hasClass('substeping')){		
						...
						if(applyTo.indexOf('.panelContainer') != -1){getTop=getTop+4; }
					...
					$('<div/>',{class:'num'}).appendTo(applyOnParent).text(inx+1).css({position:'absolute',left:getLeft+'px',top:getTop-14+'px',width:wid+'px',height:hei+'px'});
