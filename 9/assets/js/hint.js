...
if($('.subpage').filter(":visible").attr('id') == 'transferguide' || $('.subpage').filter(":visible").attr('id') == 'transferguideSim' )		
       {			
	      startToPlay('transferguide', 0)		
		  $('#closePopup').css({top: '100px','right':'192px'});		
   ...
   if(alphabeticSimulationvisite && currentidarr.length == 0)
   {
	  ...
	  if($('.subpage').filter(":visible").attr('id') == 'sitemap' || $('.subpage').filter(":visible").attr('id') == 'help'){			  
	        ...
	        $('#closePopup').css({top: '85px','right':'90px'});			
	  }
...
var clsCnfPopup = function() {alert()
