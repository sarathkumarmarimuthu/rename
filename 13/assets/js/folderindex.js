...
var pushUser = function() {
	...
	$('.userPanel,	.correctPanel, .codeResult,.orderResult').empty();	
		...
		if (typeof details.crossAns !== 'undefined' && details.job === 2) {
            if (typeof details.crossUnit !== 'undefined' && details.crossUnit.length < details.crossAns.length) {   // number of cross units is less than the number of answers
...
var showOrders = function (index, inx, getFolOdr, getInsOdr, order, val, noXRefFlag) {
	...
	var rigFol = $('.dragtooldiv').eq(getFolOdr).find('.addfolder').text();
	 ...
	 function makeInternalMapping() {
        ...
        // For Job 13, there is only one guide at index 0 and no folders to add
        // thus, we start at index one and map all the folder names

        /**
         * Job 13 has the folders listed in REVERSE from recordIndex
         * i.e., the ILLINOIS divider ("G" folder) is last (index 7) in recordIndex, but first (index 0)
         * in foldertypeandName
         *
         * @return {Object} - A Set containing the indices
         */
        var reverse_index = 0;
        for (var j = foldertypeandName.length - 1; j > 0; j--) {        // This way we leave off the zero index, the ILLINOIS divider
            mappingObject[reverse_index] = foldertypeandName[j];
            reverse_index++;
        }
    ...
    var folder_label = internalMapping[index];

    // Re-assign folder label based upon the mapping
    fol = folder_label;
	//console.log(guide);
	crscoreassign(val, 0);
