...
var chngQues = function(currQues) {
	if(uniqArr.length > 0)
	{
	$('.simbtn,.crossbtn, .hintbtn, .generalinfobtn').css('background','#5a4a42').css('cursor','pointer').css('opacity',1);
	//$('.dummySubbtn').show();
	...
	$('.simworkcontainer .choseactn').html((curVal+1)+' / '+dataRecd.length);
...
crosCount = closestOnDrop(crosCount);	
	...
	if (!(dataRecd[curVal].crossUnit.length>0))	{
		$('.simworkcontainer').css({'left':'-15px'});		
