...
//	console.log('ssssssssssssss '+cloneArr);
	...
	if(!dragging){
		...
		if($('.parentCardPanel').is(":visible")){			
			...
			if(cloneArr[cloneCnt].indexOf('m') != -1){
				...
				setTimeout(function(){$('#zoompopup #cloneContainer').find('#close2').css({'top':'20px','right':'278px'});},5)
			}else{
		...
		$.each(newArr,function(order,details){					
			...
			if (typeof details.crossUnit != 'undefined') {      // A cross reference exists for the card				
							...
							if(vv.job ==1)
                             ...
                             if ((typeof getOrg.crossAns != 'undefined') && ii < getOrg.crossAns.length) {	// One of the possible proper cross reference created and its not a duplicate
