...
//			alert(currentidarr);
	...
	{	
		...
		var start = new Date;
			...
			// minvar = Math.floor(takentime/60).toFixed(0)<10?'0'+Math.floor(takentime/60).toFixed(0):Math.floor(takentime/60).toFixed(0);
			// secvar = (takentime%60).toFixed(0)<10?'0'+(takentime%60).toFixed(0):(takentime%60).toFixed(0);
			// recordSimTimer = minvar+':'+secvar;
...
var codeForScoring = function(){
        ...
        var folderLabelArray = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q',
            ...
            '99 ELECTRONIC REPAIR SHOP', 'INTERNATIONAL COMPUTING CENTER', 'INVOICES'];

        var folderLabelSet = new Set(folderLabelArray);

        /**
         * For Job 6, Map folder letters to indices in recordIndex using foldertypeandName, so we can map them
        ...
        /**
         * For Job 6, define the folder Indices that do not have cards (type 'G' and 'S') using foldertypeandName.
         *
        ...
        function makeFolderLabels() {
                        ...
                        for (var k = 0; k < recordIndex[q].length; k++) {	//loop through all the cards in the folder
                            ...
                            if (recordIndex[q][k].hasOwnProperty('ansOrder')) {
                                var prev_job_card_obj = {
                                    ...
                                    // 'ans_loc': recordIndex[q][k].ansOrder
                                };
                                ...
                                if (recordIndex[q][k].hasOwnProperty('crossUnit')) { // check for cross ref for the card
                                    if (recordIndex[q][k].crossUnit.length > 0) {
                                        for (var x = 0; x < recordIndex[q][k].crossUnit.length; x++) {
                                            // console.log('Card ' + q + ', ' + k + ' Cross Ref: ' + x);
                                            // console.log(recordIndex[q][k].crossUnit[x].folderLocation);
                                            var cross_ref_answer_location = [-1, -1];
                                             if (recordIndex[q][k].hasOwnProperty('crossAns') && x < recordIndex[q][k].crossAns.length) {
                                                //cross_ref_answer_location = [convertLabelToLevIndex(recordIndex[q][k].crossAns[0][1][0]), recordIndex[q][k].crossAns[0][1][1]];
												cross_ref_answer_location = [convertLabelToLevIndex(recordIndex[q][k].crossAns[x][1][0]), recordIndex[q][k].crossAns[x][1][1]];
                                            }
											else{
                                                cross_ref_answer_location = [-1, -1];
                                            }
                                            var prev_job_cross_ref_obj = {
                                               // 'folder_loc': [convertJobIndexToLevIndex(recordIndex[q][k].crossUnit[0].folderLocation[0]), recordIndex[q][k].crossUnit[0].folderLocation[1]],
                                                ...
                                                //'ans_loc': [mapping[cross_ref_answer_location[0]], cross_ref_answer_location[1]],
            ...
            // Job 6 has an extra object attribute, folder_mapping
      ...
      // Set URL and Token for scoring based upon location
        ...
        if (scoring_location == "localhost"){
            ...
            scoring_token = "b5bc847ba663f192da8fb4e5f061244ec5cb68ad";
        }
        ...
        function getStuff(json_string) {
            return $.ajax({
                ...
                datatype: 'json',
                data: {
                    'api_key': scoring_token,  // external staging api key
                    'job_id': 'Sorting_06',
...
var myPdf= function(){	
        ...
        $('<div/>',{class:'pdfHead'}).appendTo('.newdiv').text('Job 6: Correspondence Filing');
