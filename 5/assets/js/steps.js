...
{	
	...
	changeCursorForNum();
	   ...
	   if( /Android|iPhone|iPad|iPod/i.test(navigator.userAgent) ) {
			$( '.snellround4 .names span, .snellroundex .names span').each(function () {
                 this.style.setProperty( 'height', '21px', 'important' );
			});
			$('#reviewAnswer .swapArea .snellround4  span,#reviewAnswer .swapArea .snellroundex  span').each(function () {
                this.style.setProperty( 'height', '21px', 'important' );
			});
	   }else{
			$('.snellround4 .names span, .snellroundex .names span').each(function () {
       ...
       if( /iPhone|iPad|iPod/i.test(navigator.userAgent) ) {
		...
		$.each(passObj[numObj],function(inx, val){							
			//$.each(val,function(i,v){
				...
				//var applyOnParent = $(applyTo+' #latteradd'+splt[0]+' .names').parent();
				var applyOnParent = $(applyTo+' #latteradd'+splt[0]+' .names').eq(splt[1]);
				//$(applyTo)
				if(inx!=0){			
					if($('.simworkarea').hasClass('scaleCard') && applyTo == '.nameing'){
						var getLeft = posOf.position().left/0.43;
						var getTop = posOf.position().top/0.43;
					}else{
					...
					//console.log(applyTo.indexOf('simworkarea')+' '+applyTo);				
						if(applyTo.indexOf('.panelContainer') != -1){
							//getTop=getTop+4; 	
							}
...
var enableEventlevel = function(mainObj,toThs,slashAns) {
	...
	//putNumber(mainObj,toThs,slashAns,i);
		$('.tlefalign1 #name_15_0 span, .tlefalign1 #name_15_1 span:first-child,.tlefalign1 #name_15_3 span:first-child,.tlefalign1 #name_15_4 span:first-child, .tlefalign1 #name_15_6 span:first-child,.noCode1 #name_1_4 span:first-child,.noCode2  #name_2_4 span:first-child, .noCode3  #name_2_2 span:first-child,.noCode4  #name_0_5 span:first-child,.noCode5  #name_1_5 span:first-child,.noCode6  #name_2_1 span:first-child,.noCode6  #name_2_2 span:first-child,.noCode7  #name_2_1 span:first-child,.noCode7  #name_2_2 span:first-child, .noCode8  #name_1_0 span:first-child,.noCode9  #name_2_0 span:first-child,.tlefalign2 #name_11_0 span,.tlefalign2 #name_11_1 span:first-child,.tlefalign2 #name_11_2 span:first-child,.tlefalign2 #name_11_3 span:first-child,.tlefalign2 #name_11_4 span:first-child,.tlefalign3 #name_16_0 span,.tlefalign3 #name_16_1 span:first-child,.tlefalign3 #name_16_4 span:first-child,.tlefalign3 #name_16_5 span:first-child,.tlefalign3 #name_16_7 span:first-child').addClass('notclick');
}
	...
	for(var i =0; i<splVal.name.length; i++){
		...
		if(notstepclass.indexOf(mainObj.alignClass[i]) == -1 )
		{
			for(var j =0; j<splVal.name[i].length; j++)
			{
				...
				if(mainObj.alignClass[i].indexOf('toBold') != -1){
					strstr = strstr.replace('To:','<b>To:</b>');
					strstr = strstr.replace('Fax:','<b>Fax:</b>');
					strstr = strstr.replace('From:','<b>From:</b>');					
				}
...
{	
	...
	convertodomtype($('.simworkarea .nameing #'+currenDivSlash).text(),$('.simworkarea .nameing #'+currenDivSlash).parent().attr('id'));
...
var convertodomtype = function(stepstr,gId)
{
	console.log(gId)
	index = 0,
...
//	console.log(dataRecd[curVal].space)	
	...
	//stepstr = stepstr.replace('To','<b>To</b>')
	var index = 0;
	while ((index = stepstr.indexOf('@', index + 1)) > 0) {
	...
	incid++;
	            if($('.simworkarea #'+gId).hasClass('toBold')){
					stepstr = stepstr.replace('To:','<b>To:</b>');
					stepstr = stepstr.replace('Fax:','<b>Fax:</b>');
					stepstr = stepstr.replace('From:','<b>From:</b>');					
				}
