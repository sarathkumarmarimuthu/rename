...
var updateNextInDrop = function(){
	...
	if (cloneArr.length!=0) {		
		...
		$('#fileHint').css({opacity:1,cursor:'pointer'});
		...
		if(cloneArr.length==0  && !chkOnDrag)
			$('[name = submitconfirmation]').removeClass('subbtn').addClass('subbtnActive');
		     $('#fileHint').off("click").css({opacity:0.5,cursor:'default'});
			$('.dummySubbtn').hide();
 ...
 if(clkOn)		
   {	
	...
	if(recordIndex[[getInx]].length>0){	
	 ...
	 if(typeof recordIndex[[getInx]] != 'undefined' && recordIndex[[getInx]].length<=3)
    {
		$('#scrollcards').hide();
    ...
    $('.cardPanel').off('touchmove').on('touchmove',function(e){
			 //e.preventDefault();
	});
}
...
var pushUser = function () {
    ...
    $.each(newArr, function (order, details) {
        ...
        createOrder(idx, details, '.orderResult', details.folderLocation[0], details.folderLocation[1], false, 0);     // Create the grades for regular cards
        idx++;
            ...
            $.each(details.crossUnit, function (ii, vv) {
                {
                    ...
                    if ((typeof getOrg.crossAns != 'undefined') && ii < 1) {	// Proper cross reference created and its not a duplicate
...
/*	// FOLDER 
	...
	// F / G / S Creation
		...
		else
		{
			...
			{
			   folderplacement += "<li><span class='wrong'>The \""+findAddedCorrectName[i][0]+"\" "+findAddedCorrectName[i][2]+" should have been placed behind  \""+findAddedCorrectName[i][3]+"\" "+findAddedCorrectName[i][5]+". </span><font> Score: 0/1</font></li>";
...
var showOrders = function(index, inx, getFolOdr, getInsOdr, order, val,noXRefFlag) {
    ...
    var right_index = 0;		
    // Find the right folder based upon the folder name		
    for (var folder_index = 0; folder_index < foldertypeandName.length; folder_index++){		
        if (foldertypeandName[folder_index][1] === getFolOdr){		
            right_index = folder_index;		
            break;		
        }		
    }		
    var rigFol = $('.dragtooldiv').eq(right_index).find('.addfolder').text();
...
var addNewFolder = function(){	
	...
	if($(this).attr('id') == 'createGuidesubbtn')
	{
		...
		folderguidname=($('#foldername').val().toUpperCase().trim().replace(/  +/g, ' '));
	    ...
	    if(folderguidname.length>55)
		{
			
	        var folderguidnameOverflow = folderguidname.substring(0,54)+"...";
			$('#dragtooldiv_'+tempid+' .addfolder').text(folderguidnameOverflow);
