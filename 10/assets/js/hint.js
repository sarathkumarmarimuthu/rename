...
var hintFuncInit = function () {
	...
	$('#filinghntclse').off('click').on('click',filinghntClosePanel);
	...
	/*	$('#hintbtnid').removeClass('hintClr');*/
		//$('#fileHint').css({background:'#fff',opacity:'1',color:'#5A4A42'});
		
	});
	$('#tooltip,#filingtooltip').click(function(e){
		e.stopPropagation();
		//$('#fileHint').css({background:'#fff',opacity:'1',color:'#5A4A42'});
	});
	...
	$( "#fileHint" ).off('click').on('click',filinghintFun);
	...
	$.each(dataRecd,function(index,value){
			      filehintaudName= 'filehint_'+index;
			      audhintSeq[filehintaudName] = [filehintaudName];
				  filehintaudName++;
	 });
}
...
var filinghintFun = function(e){ 
	//alert("click")
    e.stopPropagation();
	
	$( "#filingtooltip" ).toggle();
	showfilinghint();
	 playfilehint = 'filehint_'+curVal;
	 startToHintPlay(playfilehint,0)
}	

var showfilinghint = function(){
	audPlay[0].pause();
...
var clsPopup = function(){
   ...
   {	
	 ...
	 //  alert(currentId+' '+currentidarr[currentidarr.length-1])
	   if((currentId!='demonstration' && currentId != 'videowalkthrough') && alphabeticSimulationvisite && currentidarr[currentidarr.length-1] != 'generalinfoSim'){
		//$('#closePopup, .subpage, .subPopUp').hide();
	   }
   }
   else
   ...
   if($('.subpage').filter(":visible").attr('id') == 'help' || $('.subpage').filter(":visible").attr('id') == 'filingRules'){		
	  ...
	  if($('.subpage').filter(":visible").attr('id') == 'generalinfo' || $('.subpage').filter(":visible").attr('id') == 'generalinfoSim')		
       {			
		  ...
		  if($('.subpage').filter(":visible").attr('id') == 'fileprocedure' || $('.subpage').filter(":visible").attr('id') == 'fileprocedureSim')		
       {			
      ...
      if($('.subpage').filter(":visible").attr('id') == 'sitemap' ){			  
		  startToPlay('stop', 0)	
          $('#closePopup').css({top: '85px','right':'94px'});				
	   } 
	...
	for(i=0; i< fillingrules.length; i++)
	{
		if(fillingrules[i].title == name)
		{
			...
			if(!(/iPhone|iPad|iPod/i.test(navigator.userAgent))) {
			  $('#iosscrollbar').removeClass('mcs-horizontal mcs-horizontal-height1').addClass('desktopTable')
			  $('#iosscrollbar1').removeClass('mcs-horizontal mcs-horizontal-height2').addClass('desktopTable')
		    }else{
				...
				$('#iosscrollbar table').attr('cellpadding','5').css('width','510px');
				$('#iosscrollbar1 table').attr('cellpadding','5').css('width','500px');;
			}
