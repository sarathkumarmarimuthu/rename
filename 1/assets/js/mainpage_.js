...
var codeForScoring = function(){	   
  ...
  //--------------- New Scoring code: Modifications by Larry start here ---------------//
        ...
        function makeFolderLabels() {
            ...
            for (var i = 0; i < dataRecd.length; i++) {	//loop through all the cards

                var card_obj = {
                    'folder_loc': dataRecd[i].folderLocation,
                    'ans_loc': dataRecd[i].ansOrder
                };

                card_output_array.push(card_obj);

                // Check for cross references on the card
                // Cross refs placed in a map to determine how to label them
                for (var j = 0; j < dataRecd[i].crossUnit.length; j++) {

                    var cross_ref_obj = {       // if the cross ref is not proper, set the answer location to [-1, -1]
                        'folder_loc': dataRecd[i].crossUnit[j].folderLocation,

                        // If the card has a crossAns, then it should have a cross reference (a proper cross ref)
                        // So, if the cross ref is proper, set the answer location to what's in crossAns
                        // If it's not proper, set the answer location to [-1, -1]
                        'ans_loc': (dataRecd[i].hasOwnProperty('crossAns')) ? dataRecd[i].crossAns[0][1] : [-1, -1],

                        'count': dataRecd[i].crossUnit[j].crosRefCount
                    };

                    cross_ref_output_array.push(cross_ref_obj);

                }
            }

            console.log(card_output_array);
            console.log(cross_ref_output_array);
            ...
            var json_string = JSON.stringify(json_output_object);
