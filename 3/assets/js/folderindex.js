...
var setCursor = function(e,ui) {	
	...
	$(ui.helper).find('.clsCrosRef').hide();		 
		...
		//$(ui.helper).find('.leveling_unit').css('cursor','move');
...
var closestOnDrop = function(closestTo) {
	clost = [];
	crosDrop=[];
//	console.log(JSON.stringify(dataRecd[curVal].crossUnit)+' revert');
		$.each(dataRecd[curVal].crossUnit,function(i,v){
			if(!v.dropped)
				clost.push(i);
			else{
...
var checkFinalCard = function() {
	...
	if (uniqArr.length!=1) {		
		...
		$('.dummySubbtn').hide();
		$('.offSteps').css('width','175px').css('height','225px').show();
		$('.crossbtnBadge').hide();
		hintshowtag = false;
		$('#tooltip').hide();
...
var checkRecdIndex = function(e, ui) {		
	changeFolderImg();
	idx=0;
	$('.panelContainer,.dummyEditclk,.editclk,.panelContainer2').remove();
	...
	$('#totalCards').text('Total Cards: '+recordIndex[[currAlphabet]].length);
	$('.panelContainer,.panelContainer2').css({transform:'scale(0.58)',transformOrigin:'left top'});
	...
	//$('.cardPanel').css({overflowY:'auto',background:'red'});
...
var zoomCard = function(){
	...
	$('#cloneContainer').empty().show();
		...
		$('#cloneContainer .leveling_unit').attr('contenteditable',false);	
	...
	$('.footer').css('z-index','unset');
		...
		$(".trimSpace").getNiceScroll().hide();
		$('#cloneContainer .swapAreaDrop').find('.editclk,.dummyEditclk').remove();
		var lenOfdivs = $('#cloneContainer .cdsname').children().length
			$('#cloneContainer .swapAreaDrop ').css({top:'40px', 'left':'auto'});
			$("#cloneContainer .swapArea ").css({'max-height':'270px'})				
			$('#cloneContainer .swapArea').css({height:'auto'});	
     
		if(lenOfdivs>0){
			$('#cloneContainer .swapArea').css({top: ((8-lenOfdivs)*10)+'px'});
			$('#close').css({top:(((7-lenOfdivs)*10)+10)+'px'});
...
var getRmvVal = function(e,ui){
	...
	toRemv = $(this).attr('moveCrd');	
	 //$('.dragtooldiv').css({visibility:'visible'});
		...
		$(ui.helper).find('.editclk,.dummyEditclk').hide();
		$('#close').hide(); 
		...
		$(this).find('.swapAreaDrop').remove();
			...
			$('.dg').text('');
		    $(ui.helper).find('.swapAreaDrop .levelingnameing').css('top','10px');
			$(ui.helper).find('*').css({cursor:'move'});
		}else{
...
var editDataClk = function(){
	...
	$('.dummyEditclk').off('touchstart mousedown').on('touchstart mousedown',editData);
   ...
   if(ctl == 1 || inDevice){	
	...
	if($(this).hasClass('crossbtnBadge') && typeof dataRecd[curVal].crossUnit != 'undefined'){	
		...
		if(typeof(recordIndex[[currAlphabet]][editDatInd].order) != 'undefined'){
			...
			//console.log(JSON.stringify(dataRecd[0].crossUnit))	;
			recordIndex[[currAlphabet]][editDatInd].dropped = false;
			
			//console.log('after'+JSON.stringify(dataRecd[0].crossUnit))	;
...
var swapInside = function(i,objVal,appendClass) {	
	...
	if(objVal.job != 1 && (typeof objVal.leveling != 'undefined' || objVal.levelingAns != 'undefined')){

			$($('.simworkarea .levelingnameing').clone()).appendTo(appendClass+' #card'+i).hide()//.show();
				
		if(appendClass.indexOf('.correctPanel') > -1 && typeof objVal.levelingAns != 'undefined'){	
			levelingUpdate(objVal.levelingAns, appendClass+' #card'+i);
			levelingCalc(objVal);
		}else if(appendClass.indexOf('.correctPanel') == -1){
//	console.log('additinal'+' '+appendClass);
			levelingUpdate(objVal.leveling, appendClass+' #card'+i);
		}
			 if(inDevice){
				 $('.swapArea .leveling_unit').css({height:'10px'});
			 }
	}
	...
	if(objVal.job == 1){
		...
		if(appendClass == ".userPanel"){
			...
			$(appendClass+' #swapAreaDrop_'+i+' .swapArea').empty().html('<span class="jobSpan">Card from Job 2</span>');
			}
	}
	temparr = [];
...
var levelingCalc = function(levObj){
		var levScrCnt = 0;

    if(typeof levObj.leveling != 'undefined'){	
        $.each(levObj.leveling,function(index,v){
	   
	  if(index == 0){
	     //if(v.replace(/\s+/g,' ').trim().toString().toLowerCase() == levObj.levelingAns[0].toString().toLowerCase()){
	     if(v.replace(/\s+/g,' ').trim().toString().replace(/  +/g, ' ')== levObj.levelingAns[0].toString().replace(/  +/g, ' ')){
				levScrCnt++; 
	...
	levObj.codeScore[1] = 1;
	levObj.codeScore[0] = levScrCnt;
    totcodescore += levObj.codeScore[1];
	//console.log('level '+totcodescore)
}

// i is the recordIndex array index for the main card
// objVal is dataRecd.crossUnit
var addCrossRef = function(i,objVal,appendClass,isAns) {
	...
	$('<div/>',{class:'dummyEditclk crossClk'}).appendTo(appendClass+' #swapArea_'+i+' .form');
...
/**		
 * Function added by Larry to compare two arrays for equality		
 *		
 * @param {Array} first_array - How long to wait		
 * @param {Array} second_array - How long to wait		
 * @return {Boolean} True if arrays are the same, False if not		
 */		
var compareArrays = function(first_array, second_array){		
    // Check that both arguments are arrays		
    if (first_array instanceof Array){		
        if (second_array instanceof Array){		
            var i = first_array.length;		
            while (i--) {		
                if (first_array[i] !== second_array[i]) return false;   // Compare all the values individually		
            }		
            return true		
        }		
        else{		
            console.log("Warning: Non-array argument used for second array in compare");		
            return false;		
        }		
    }		
    else{		
...
//------------- Modifications made by Larry in pushUser -------------------//
...
var pushUser = function () {
    ...
    idx = 0;
   $.each(newArr, function (order, details) {  // loop through the combined array of normal cards
        ...
        createOrder(idx,details,'.orderResult',details.folderLocation[0],details.folderLocation[1], false, 0);
        idx++;

        if (typeof details.crossUnit != 'undefined') {  // loop through the main cards that have cross refs
            $.each(details.crossUnit, function (ii, vv) {   // loop through the cross references for the main cards
                {
					// ii  -- the index of the cross unit
					// vv  -- dataRecd.crossUnit[ii]
					
                    var getCrossAns = vv.arr[9].split(','), getOrg; // vv is the cross ref card: location 9 is a string representing the location of the original card
                    var crossAnsOdr = getCrossAns[1];
                    if (vv.job == 1)
                        getOrg = dataRecdJob[crossAnsOdr];  // getOrg is the main card the cross ref comes from
                    else
                        getOrg = dataRecd[crossAnsOdr];


                     // There is supposed to be at least one cross ref (because crossAns exists)
					 // We are looping through ALL the cross refs created if there is at least an answer for one
					 
                    // if ((typeof getOrg.crossAns != 'undefined') && ii < 1) {	//added check for multiple cross refs being added
                    if ((typeof getOrg.crossAns !== 'undefined') && (ii < getOrg.crossAns.length)) { // current index is in the answer set
                         addCrossRef(idx, vv, '.userPanel', 'notAns');
                         if (ii === vv.ansMatchWith){       // The cross unit index matches the answer (this is correct)
                            ...
                            createOrder(idx, vv, '.orderResult', vv.folderLocation[0], vv.folderLocation[1], false, ii);
                        }
                        else{
                            if (vv.ansMatchWith !== -1) { // An answer exists for the specified cross reference, but it isn't matched with the correct cross ref -- use the matched cross ref instead
                                var crossUnitForCorrectAnswer = 0;
                                for (var ans_idx=0; ans_idx < details.crossUnit.length; ans_idx++){
                                    if (ii === details.crossUnit[ans_idx].ansMatchWith){
                                        crossUnitForCorrectAnswer = details.crossUnit[ans_idx];
                                        break;
                                    }
                                }
                                addCrossRef(idx, crossUnitForCorrectAnswer, '.correctPanel', 'refAns');
                                createCrossAns(idx, vv, '.codeResult', 'crosRef', vv.folderLocation[0], vv.folderLocation[1], getOrg);
                                createOrder(idx, vv, '.orderResult', vv.folderLocation[0], vv.folderLocation[1], false, ii);
                            }
                            else {  // No answer for the cross ref -- should have an empty spot
                                addCrossRef(idx, details.crossUnit[ii], '.correctPanel', 'refAns', 'empty');
                                createCrossAns(idx, vv, '.codeResult', 'notCrosRef', vv.folderLocation[0], vv.folderLocation[1], getOrg);
                                createOrder(idx, vv, '.orderResult', vv.folderLocation[0], vv.folderLocation[1], true, ii);
        ...
        // Check for missing cross references
         if (typeof details.crossAns !== 'undefined' && details.job === 2) {
            if (typeof details.crossUnit != 'undefined' && details.crossUnit.length < details.crossAns.length) {
				...
				// Take care of the missing cross references
                for (var i = details.crossUnit.length; i < details.crossAns.length; i++) {
                    ...
                    createOrder(idx, details.crossUnit[i], '.orderResult', details.crossUnit[i].folderLocation[0], details.crossUnit[i].folderLocation[1], true, i);
...
var createCode = function (inx, val, appclass) {
    ...
    if (val.job == 1) {
        ...
        levelScore(inx, val, appclass)
    }

}
var levelScore = function (inx, val, appclass) {

    if (typeof val.leveling != 'undefined' && typeof val.levelingAns != 'undefined') {

        if (val.codeScore[0] == val.codeScore[1]) {

            $('#cardIn_' + inx).html('<span class="right">You coded this card correctly. <br><font color="#000" >Note that coding on Levels 2 and 3 is not graded.</font><br><font color="#000">Score: 2/2</font></span>');
            codescore++;
            //console.log(codescore +'corr '+ val.codeScore[0])
        } else {


            $('#cardIn_' + inx).html('<span class="wrong">Your coded card contains errors. <br><font color="#000" >Note that coding on Levels 2 and 3 is not graded.</font><br><font color="#000">Score: 1/2</font></span>');
            codescore++;
            //console.log(codescore +'incorr '+ val.codeScore[0])
        }

    } else if (typeof val.levelingAns != 'undefined' && typeof val.leveling == 'undefined') {
        $('#cardIn_' + inx).html('<span class="wrong">Your coded card contains errors.<br><font color="#000">Score: 0/2</font></span>');
    } else if (typeof val.leveling != 'undefined') {
        $('#cardIn_' + inx).html('<span class="wrong">Your coded card contains errors.<br><font color="#000">Score: ' + val.codeScore[0] + '/' + val.codeScore[1] + '</font></span>');
    }
}
...
// val is the cross ref from dataRecd.crossUnit		
// getOrg is the location of the main card in dataRecd		
// afol is the cross ref folder location		
// ainx is the cross ref order location
...
//   inx is the index in the recordIndex array (contains all the cards and cross refs)
//  val is dataRecd or dataRecd.crossUnit
//  appclass is '.orderResult' -- the place in the html where things should appear
//  index is dataRecd or dataRecd.crossUnit folderLocation[0] -- the folder where the card actually is
// order is dataRecd or dataRecd.crossUnit folderLocation[1] -- the order where the card actually is in the folder
...
//  crossRefIndex is an int that is the dataRecd.crossUnit index -- this was added by Larry
var createOrder = function (inx, val, appclass, index, order, noXRefFlag, crossRefIndex) {
    ...
    if (typeof val.ansOrder !== 'undefined') {  // Normal Cards all have ansOrder
	...
	else if (typeof val !== 'undefined') {    // Cross references only from this point
        var getOrg;
        var getCrossAns = val.arr[9].split(',');
        var crossAnsOdr = getCrossAns[1];       // Location of the main card in the dataRecd array
        ...
        //if(typeof val.ansMatchWith != 'undefined')
        {
            ...
            if (val.job == 1) {
               var getFolOdr = getOrg[val.ansMatchWith][1][0];     // The index of the folder of the answer
               var getInsOdr = getOrg[val.ansMatchWith][1][1];     // The order in the folder of the answer
                ...
                if (typeof val.ansMatchWith != 'undefined' && typeof getOrg[val.ansMatchWith] != 'undefined') {
                    if (noXRefFlag){    // A cross ref with no answer has been submitted
                        getFolOdr = -1;
                        getInsOdr = -1;
                    }
                    else if (val.ansMatchWith !== -1){    // Only for cross refs that have answers
                        if (crossRefIndex < getOrg.length) {
                            getFolOdr = getOrg[crossRefIndex][1][0];  // Changed because if there are multiple cross refs, they may be mis-ordered
                            getInsOdr = getOrg[crossRefIndex][1][1];
                        }
                        else {   // In case extra cross refs have been created
...
// val -- dataRecd.crossUnit		
// getOrg -- dataRecd.crossAns		
// index -- dataRecd.crossUnit.folderLocation[0]		
// order -- dataRecd.crossUnit.folderLocation[1]		
// job -- 1 for existing cards (cards from previous jobs)
...
//  inx is the index of the card in the recordIndex array
