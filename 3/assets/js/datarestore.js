...
var leveltag =false;
var levelingUpdate = function(getMainObj, toPlace){
	if(typeof getMainObj != 'undefined'){
	leveltag =true;
	if(toPlace.indexOf('.simworkarea') == -1)
	{
		//$(toPlace+' .leveling_unit').attr('contenteditable',false);	
	}
	$(toPlace+' .levelingnameing').show().css({visibility:'visible'});
   if(toPlace.indexOf('.simworkarea') != -1){
	   if(dataRecd[curVal].name.length==6){
		$('.simworkarea').css({height:'282px',top:'15px'});
		$('.simworkcontainer > p').css({marginTop: '310px'});
		$('.crossbtnBadge').css({top: '310px'}); 
		$('.simworkAreaOff').css({top: '-9px'}); 
	   }else{ $('.simworkarea').css({height:'260px'});
	          $('.simworkcontainer .choseactn').css({marginTop:'312px'}); 
			  $('.crossbtnBadge').css({top: '312px'}); 
			   $('.simworkAreaOff').css({top: '9px'});
	   }  

   }
	   $(toPlace+' .leveling_unit').val('');
//		console.log(toPlace+' '+JSON.stringify(getMainObj));
		//if(typeof getMainObj.inputArr != 'undefined')
		{
			$.each(getMainObj,function(i,v){			
				$(toPlace+' .leveling_unit').eq(i).val($.trim(v));
				$(toPlace+' .leveling_unit').eq(i).attr('value',$.trim(v));
			});			
		}

	}else{
//		console.log(toPlace)
		$(toPlace+' .levelingnameing').hide();
		
      if(toPlace.indexOf('.simworkarea') != -1){
        if(dataRecd[curVal].name.length==6){
		  
		  $('.simworkarea').css({height:'188px',top:'38px'});
          $('.simworkcontainer > p').css({marginTop: '255px'});
		  $('.crossbtnBadge').css({top: '255px'});
		  $('.simworkAreaOff').css({top: '9px'});
	    }else{ 	
		  $('.simworkarea').css({height:'163px',top:'38px'});
...
var keyPressLevelng = function(){
    
	$('.leveling_unit').keypress(function(e){ return e.which != 13; });
	
	$('.leveling_unit').off('keyup change').on('keyup change',function(e,ui){
			var insertUnit = $(this).parent().index();
			if(typeof dataRecd[curVal].leveling == 'undefined'){
								
				dataRecd[curVal].leveling = new Array();
    				
			}				
			dataRecd[curVal].leveling[insertUnit] = ( $(this).val());
			
			//console.log(dataRecd[curVal].leveling+' '+insertUnit);					
	});
}
...
var splitName = function(getName,addClass,addLabel,getMainObj) {
