...
var putNumber = function(passObj,applyTo,slashAns){		//alert($(applyTo).parent().attr('class'));
		...
		if(applyTo == '.nameing'){
			$('.simworkarea .num').remove();
			$('.simworkarea .levelingnameing').css('display','none');
		}	
			if(slashAns == 'answer')
				numObj = 'anumbering';
			else	
...
var removeSlash = function(){
	//$('.simworkarea .nameing em').html('&nbsp;');
	
	dataRecd[curVal].slash = [];
	$('.simworkarea .nameing .names').each(function(i,v){
		
		$(v).find('em').each(function(i,v){
	
			//&& $(v).attr('class').indexOf('p') == -1
			if($(v).html() == '/' && typeof $(this).attr('class') == 'undefined'){
				$(v).html('&nbsp');
			}else if($(v).html() == '/' && typeof $(this).attr('class') != 'undefined'){
				$(v).html('');
			}
		});
		currentDiv = i
		convertodomtype($('.simworkarea .nameing #name_'+i).text());
	});
...
var convertodomtype = function(stepstr)
{
	...
	for(i=0; i<slashcount.length; i++)
	{
		...
		//alert(dataRecd[curVal].slash[currentDiv].push(dataRecd[curVal].space[currentDiv].indexOf(slashcount[i])))
