...
var asciicodestartval;
	...
	$(' .sitemapbtn, .fbtn, .btn, .subbtn, .backBtn, #startbtn, .clkLink').bind('click', buttonEventCalling);
	$('.crossbtn').bind('click', crossBtnEventCalling);
	$('.simbtn').bind('click', alpRulesselectfun);
	//$('.levelbtn').bind('click', levelingrFileBtnEventCalling);
...
$('.leveling_unit').off('focusin').on('focusin', function(e){	
	$('.tcent .mCSB_container').html(simaction.leveling);
	if(titleNew !== 'leveling')
	{
	startToPlay('leveling',0,'le');
	}
});
...
var alpRulesselectfun = function() {
	...
	if($(this).css('opacity') ==1)
	{
		...
		if (nameIdTemp == currentId && currentId!='crossreference' && nextPreTag ) {
			...
			//$('.leveling_unit').attr('contenteditable',false);	
		...
		$('em,span').off('mousemove');
		 delete dataRecd[curVal].leveling;
	levelingUpdate(dataRecd[curVal].leveling,'.simworkarea');
			...
			if(currentId != 'leveling')
			 ...
			 else if(currentId == 'uIdentification')
			 {
				...
				$('.leveling_unit').attr('contenteditable',true);	
				levelingrFileBtnEventCalling();
			 }
...
var levelingrFileBtnEventCalling = function(){		
     removeNumbering();   
	   removeSlash(); 
	  
	if(typeof dataRecd[curVal].leveling == 'undefined')
	{	

		dataRecd[curVal].leveling = {};
		keyPressLevelng();
	}else{
		$('.levelingnameing').hide();
...
var delClk = function(){
		...
		if(dataRecd[curVal].crossUnit.length ==0){	
			crossGhostimgRemove();
  if($('[name=uIdentification]').css('background-color') == 'rgb(90, 74, 66)' && $('[name=Code]').css('background-color') == 'rgb(90, 74, 66)' && $('[name=leveling]').css('background-color') == 'rgb(90, 74, 66)')
			{
				$('.tcent .mCSB_container').html(simaction.alphabeticSimulation)
				  startToPlay('alphabeticSimulation', 0)
...
//			alert(currentidarr);
		...
		if((currentId == 'help' || currentId == 'sitemap' || currentId == 'filingRules') && newArr.length==0){
...
var codeForScoring = function(){
        ...
        function makeFolderLabels() {
            ...
            // Take care of the cards and cross refs from Job 01
                    ...
                    for (var k = 0; k < recordIndex[q].length; k++) {	//loop through all the cards in the folder
                        ...
                        if (recordIndex[q][k].hasOwnProperty('ansOrder')) {
                            var prev_job_card_obj = {
                                'folder_loc': recordIndex[q][k].folderLocation,
                                    ...
                                    for (var x = 0; x < recordIndex[q][k].crossUnit.length; x++) {
                                        ...
                                        var cross_ref_answer_location = [-1, -1];
                                         if (recordIndex[q][k].hasOwnProperty('crossAns') && (x < recordIndex[q][k].crossAns.length)){
                                           cross_ref_answer_location = recordIndex[q][k].crossAns[x][1];
                                        }
                                        var prev_job_cross_ref_obj = {
                                            'folder_loc': recordIndex[q][k].crossUnit[x].folderLocation,
                                            'ans_loc': cross_ref_answer_location,
                                            'count': x
                                        };

                                        cross_ref_output_array.push(prev_job_cross_ref_obj);
                                        // console.log('after cross ref push');
                                    }
                                }
                            }
                        }
        ...
        // var folder_labels;
        /**
         *  There is a race condition occuring (we think) that causes the cross unit folder locations to not be
         *  set prior to making the folder labels.  Thus, we want to wait unit the locations are set.
         *
         * @return {String} - The json string with the cards and cross refs
         */
        function waitForCrossUnits(){
            var num_cross_refs = 0;
            var missing_folder_location_flag = false;
            for (var q = 0; q < recordIndex.length; q++) {  //Loop through folders
                if ((recordIndex[q] !== null) && typeof recordIndex[q] !== 'undefined') {
                    for (var k = 0; k < recordIndex[q].length; k++) {	//loop through all the cards in the folder
                        if (recordIndex[q][k].hasOwnProperty('crossUnit')) { // check for cross ref for the card
                            num_cross_refs += recordIndex[q][k].crossUnit.length;
                            for (var x = 0; x < recordIndex[q][k].crossUnit.length; x++) {
                              //  console.log(recordIndex[q][k].crossUnit[x]);
                                if(!recordIndex[q][k].crossUnit[x].hasOwnProperty('folderLocation')){
                                    missing_folder_location_flag = true;
                                    break;
                                }
                            }
                            if (missing_folder_location_flag){
                                break;
                            }
                        }
                    }
            ...
            if (missing_folder_location_flag){
                setTimeout(waitForCrossUnits, 250);
            }
            else{
                return makeFolderLabels();
            }
        }
        // Make the labels
        var folder_labels = waitForCrossUnits();
      //  console.log(folder_labels);
        ...
        getStuff(folder_labels).done(function (result) {
			...
			//console.log(JSON.stringify(scoreAjaxData))
            pushUser();
...
//		console.log(tempstr)
	...
	for(var i = 0; i<dataRecd[curVal].numbering.length; i++)
	{
		...
		if(typeof dataRecd[curVal].anumbering[i] != 'undefined')
		{
			...
			if(userAns.replace(/\s/g,'#')==relAns.replace(/\s/g,'#') && dataRecd[curVal].numbering[i] == dataRecd[curVal].anumbering[i])
			{
				count++;
			}	
		}
...
var myPdf= function(){	
        ...
        $('<div/>',{class:'pdfHead'}).appendTo('.newdiv').text('Job 3: Alphabetic Filing Rules');
